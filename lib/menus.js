...
const transferStarsOtherAccountMenu = [
  {
    type: 'input',
    name: 'transferFromUsername',
    message:
      'Please specify the username of the GitHub account you would like to transfer stars FROM:',
    validate: function(value) {
      if (value === conf.get('username')) {
        return `Please enter a different account than the current account, or go back and select transfer stars from ${chalk.green(
          conf.get('username')
        )}.`
      } else {
        if (value.length) {
  ...
  {
    type: 'password',
    name: 'transferFromPassword',
    message:
      'Please specify the password of the GitHub account you would like to transfer stars FROM:',
    validate: function(value) {
      if (value.length) {
        return true
      } else {
...
const transerStarsCurrentAccountResponse = (answer) => {
  // Set temporary user credentials
  conf.set('tempUsername', answer.transferToUsername)
  conf.set('tempPassword', answer.transferToPassword)

  // Transfer stars from Current User to Temporary User
  transferStars()
}

const transerStarsOtherAccountResponse = (answer) => {
  // Set temporary user credentials
  conf.set('tempUsername', answer.transferFromUsername)
  conf.set('tempPassword', answer.transferFromPassword)

  // Transfer stars from Temporary User to Current User
  transferStars('temp')
}
...
const transferStarsReponse = (answer, questions) => {
  ...
  const otherAccountResponse = () => {
    console.log('')
    createMenu(
      transferStarsOtherAccountMenu,
      transerStarsOtherAccountResponse,
      'transfer'
    )
  }
